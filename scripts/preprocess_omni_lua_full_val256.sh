...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/data/prepared/omni_lua_sft_full_val256}"
VAL_COUNT="${VAL_COUNT:-256}"
VAL_MAX_AUDIO_SECONDS="${VAL_MAX_AUDIO_SECONDS:-30}"
SEED="${SEED:-42}"
SMOKE_REPORT="${SMOKE_REPORT:-$ROOT_DIR/outputs/omni_lua_full_val256_preprocess_smoke.json}"

echo "Building full Omni Lua SFT dataset (fixed val count)"
echo "  audio_dir:    $AUDIO_DIR"
echo "  lua_dir:      $LUA_DIR"
echo "  prompt_file:  $PROMPT_FILE"
echo "  out_dir:      $OUT_DIR"
echo "  val_count:    $VAL_COUNT"
echo "  val_max_sec:  $VAL_MAX_AUDIO_SECONDS"
echo "  seed:         $SEED"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/build_omni_lua_sft_dataset.py" \
  ...
  --out-dir "$OUT_DIR" \
  --val-count "$VAL_COUNT" \
  --val-max-audio-seconds "$VAL_MAX_AUDIO_SECONDS" \
  --seed "$SEED" \
  --require-lua-markers
